...
public final class GenerateSharedTestTargetApphostFilesWorkspaceMapper {
    private let derivedDirectoryName: String
    private let sourcesDirectoryName: String

    public init(
        derivedDirectoryName: String? = nil,
        sourcesDirectoryName: String? = nil
    ) {
        self.derivedDirectoryName = derivedDirectoryName ?? Constants.DerivedDirectory.name
        self.sourcesDirectoryName = sourcesDirectoryName ?? Constants.DerivedDirectory.sources
    }
       ...
       var sideEffects: [SideEffectDescriptor] = []
        ...
        for p in 0 ..< workspace.projects.count {
            ...
            for t in 0 ..< project.targets.count {
                ...
                guard
                    sideTable.targetFlags(path: project.path, name: targetName)
                ...
                workspace.projects[p].targets[t].sources = [SourceFiles(paths: sourceFiles)]
                workspace.projects[p].targets[t].resources = [ResourceFileElement.file(path: storyboardPath)]
