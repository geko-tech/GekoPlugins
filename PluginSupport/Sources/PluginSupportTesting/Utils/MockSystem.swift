...
public final class MockSystem: Systeming {
    ...
    public func run(_ arguments: [String], redirection: PluginSupport.Process.OutputRedirection) throws {
        _ = try capture(arguments, verbose: false, environment: [:])
    }
    
    public func run(_ arguments: [String], verbose: Bool, environment: [String : String], redirection: PluginSupport.Process.OutputRedirection) throws {
    ...
    public func capture(_ arguments: [String], verbose _: Bool, redirectStdErr: Bool, environment _: [String: String]) throws -> String {
        ...
        guard let stub = stubs[command] else {
            throw PluginSupport.SystemError.terminated(command: command, code: 1, standardError: Data(), standardOutput: Data())
        }
        if stub.exitstatus != 0 {
            throw PluginSupport.SystemError.terminated(
    ...
    public func runAndCollectOutput(_ arguments: [String]) async throws -> SystemCollectedOutput {
        let command = arguments.joined(separator: " ")
        guard let stub = stubs[command] else {
            throw PluginSupport.SystemError
    ...
    public func async(_ arguments: [String]) throws {
        let command = arguments.joined(separator: " ")
        guard let stub = stubs[command] else {
            throw PluginSupport.SystemError.terminated(command: arguments.first!, code: 1, standardError: Data(), standardOutput: Data())
        }
        if stub.exitstatus != 0 {
            throw PluginSupport.SystemError.terminated(command: arguments.first!, code: 1, standardError: Data(), standardOutput: Data())
        }
    ...
    public var chmodStub: ((FileMode, PluginSupport.AbsolutePath, Set<FileMode.Option>) throws -> Void)?
    public func chmod(
        _ mode: FileMode,
        path: PluginSupport.AbsolutePath,
