...
public enum FileHandlerError: FatalError, Equatable {
    ...
    public var type: ErrorType {
        ...
        case .invalidTextEncoding:
            return .bug
        case .writingError, .fileNotFound, .unreachableFileSize, .unreachableFileAccessDate, .expectedAFile:
            return .abort
        }
    }
}
...
public protocol FileHandling: AnyObject {
    ...
    func write(_ content: String, path: AbsolutePath) throws
    ...
    func allContentsOfDirectory(_ path: AbsolutePath) throws -> [AbsolutePath]
...
public class FileHandler: FileHandling {
    ...
    public var currentPath: AbsolutePath {
        try! AbsolutePath(validatingAbsolutePath: fileManager.currentDirectoryPath)  // swiftlint:disable:this force_try
    }
    ...
    public func write(_ content: String, path: AbsolutePath) throws {
        try write(content, path: path, atomically: true)
    }
    ...
    public func throwingGlob(_ path: AbsolutePath, glob: String) throws -> [AbsolutePath] {
        ...
        return try path.throwingGlob(glob)
    }
    ...
    public func contentsOfDirectory(_ path: AbsolutePath) throws -> [AbsolutePath] {
        ...
        return try fileManager.contentsOfDirectory(atPath: path.pathString).map { try AbsolutePath(validating: $0, relativeTo: path) }
    }

    public func allContentsOfDirectory(_ path: AbsolutePath) throws -> [AbsolutePath] {
        let allPaths = try fileManager.subpathsOfDirectory(atPath: path.pathString)
        let fullPaths = allPaths.map { AbsolutePath(stringLiteral: "\(path.pathString)/\($0)") }
        return fullPaths
    }
    ...
    public func resolveSymlinks(_ path: AbsolutePath) throws -> AbsolutePath {
        ...
        #if canImport(PluginSupport)
        return try PluginSupport.resolveSymlinks(path)
        #else
        return try PluginSupportStatic.resolveSymlinks(path)
        #endif
    }
    ...
    public func filesAndDirectoriesContained(in path: AbsolutePath) throws -> [AbsolutePath]? {
        ...
        return try fileManager.subpaths(atPath: path.pathString)?.map { path.appending(try RelativePath(validating: $0)) }
    }
